...
{
    @Query("SELECT t FROM Token t WHERE t.user_email =:email")
    public Token findTokenByEmail(@Param("email") String email);
    @Query("SELECT t FROM Token  t WHERE t.token_expiration_time < :expirationTime")
    public List<Token> findTokenByExpirationTime(@Param("expirationTime") LocalDateTime expirationTime);
    @Query("SELECT t FROM Token t WHERE t.token_id =:tokenID")
